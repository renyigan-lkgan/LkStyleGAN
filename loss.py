...
#----------------------------------------------------------------------------
# LKGAN loss functions.
def G_lkgan(G, D, opt, training_set, minibatch_size, k=1.0, gamma=0.5): # pylint: disable=unused-argument
    latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    labels = training_set.get_random_labels_tf(minibatch_size)
    fake_images_out = G.get_output_for(latents, labels, is_training=True)
    fake_output = fp32(D.get_output_for(fake_images_out, labels, is_training=True))
    loss = tf.math.reduce_mean(tf.math.pow(tf.math.abs(fake_output - gamma),
                                               k * tf.ones_like(fake_output)))
    return loss

def D_lkgan(G, D, opt, training_set, minibatch_size, reals, labels, 
        alpha=1.0, beta=0.0):
    r1_gamma = 10.0
    latents = tf.random_normal([minibatch_size] + G.input_shapes[0][1:])
    fake_images_out = G.get_output_for(latents, labels, is_training=True)
    real_output = fp32(D.get_output_for(reals, labels, is_training=True))
    fake_output = fp32(D.get_output_for(fake_images_out, labels, is_training=True))
