...
if 1:
    ...
    G_loss        = EasyDict(func_name='training.loss.G_lkgan', k=1.0, gamma=1.0)                 # Options for generator loss.
    D_loss        = EasyDict(func_name='training.loss.D_lkgan', alpha=0.0, beta=1.0) # Options for discriminator loss.
    dataset       = EasyDict()                                                             # Options for load_dataset().
    sched         = EasyDict()                                                             # Options for TrainingSchedule.
    grid          = EasyDict(size='4k', layout='random')                                   # Options for setup_snapshot_image_grid().
    metrics       = [metric_base.fid50k]                                                   # Options for MetricGroup.
    submit_config = dnnlib.SubmitConfig()                                                  # Options for dnnlib.submit_run().
    tf_config     = {'rnd.np_random_seed': 3000}                                      # Options for tflib.init_tf().
    ...
    # Number of GPUs.
    desc += '-1gpu'; submit_config.num_gpus = 1; sched.minibatch_base = 4; sched.minibatch_dict = {4: 128, 8: 128, 16: 128, 32: 128, 64: 128, 128: 128, 256: 128, 512: 128}
